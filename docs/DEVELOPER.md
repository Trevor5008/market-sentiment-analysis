...
│   └── processed/              # Cleaned outputs (overwritten each run)
├── scripts/
│   ├── data_ingestion.py       # Fetch GDELT articles + OHLCV prices; archive previous; write run manifest
│   ├── validate_gdelt.py      # Validate GDELT; report to docs/validation/
│   ├── cleaning_gdelt.py       # Clean GDELT → data/processed/gdelt_articles_clean.csv
│   ├── ohlcv_validation.py    # Validate OHLCV; report to docs/validation/
│   ├── ohlcv_cleaning.py      # Clean OHLCV → data/processed/prices_daily_clean.csv
│   └── run_pipeline.sh        # Orchestrates validation + cleaning; optional ingestion (RUN_INGEST=1)
├── notebooks/                  # Exploratory analysis (use cleaned data only)
├── docs/
...
| `cleaning_gdelt.py` | Dedupe, filter language/relevance, drop cols; overwrites output | `data/processed/gdelt_articles_clean.csv` |
...
| `run_pipeline.sh` | Run validation + cleaning; optionally run ingestion when `RUN_INGEST=1` | Depends on steps run |
